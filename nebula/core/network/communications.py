...
class CommunicationsManager:
    ...
    async def handle_incoming_message(self, data, addr_from):
        try:
            ...
            if message_wrapper.HasField("discovery_message"):
                logging.info("incoming message is discovery_message")
                ...
                logging.info("incoming message is connection_message")
    ...
    async def deploy_network_engine(self):
        ...
        self.network_engine = await asyncio.start_server(
            self.handle_connection_wrapper, self.host, self.port
        )  # host is reader (recieves data from client) and port is writter (send data to client)
